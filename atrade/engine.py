...
def _shared_research(st, cfg, force_mock: bool, session: str = "open",
    ...
    return summary, price_map, broker, mode, tech
...
def open_run(state_dir: str | Path, force_mock: bool = False, allow_anyday: bool = False,
    ...
    summary, price_map, broker, mode, tech = _shared_research(st, cfg, force_mock, session="open",
    ...
    hyps = signals.build_hypotheses(summary, tech, tracker, cfg.get("universe", []), cfg)
    ...
    try:
        ...
        msg = telegram.format_open(util.utc_iso(), mode, len(cfg.get("universe", [])),
                                   opened, skipped, hyps)
        telegram.send(msg)
    except Exception as e:
...
def close_run(state_dir: str | Path, force_mock: bool = False, allow_anyday: bool = False,
    ...
    summary, price_map, broker, mode, tech = _shared_research(st, cfg, force_mock, session="close",
                                                               use_cached=use_cached)
    tracker = st.data.get("signal_tracker") or {}
    hyps = signals.build_hypotheses(summary, tech, tracker, cfg.get("universe", []), cfg)
    ...
    if eval_res.get("pause"):
        st.mark_paused(f"no_improve_streak={eval_res['no_improve_streak']} | "
                       f"failed_measure={eval_res['failed_measure_streak']}")
        try:
            from . import telegram
            telegram.send(telegram.format_paused(
