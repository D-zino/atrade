...
def update_signal_tracker(tracker: dict, closed: list[dict], decay: float = 0.92) -> dict:
    ...
    # per-symbol learning (Tier 2): which names' setups actually pay
    syms = tr.setdefault("_symbols", {})
    for t in closed:
        sym = t.get("symbol")
        win = t.get("hypothesis_correct", 0)
        st = syms.setdefault(sym, {"n": 0, "wins": 0, "edge": 0.0})
        st["n"] += 1
        st["wins"] += win
        st["edge"] = round(st["edge"] + (t.get("pnl") or 0), 2)
        st["win_rate"] = round(st["wins"] / st["n"], 3)
    # keep only symbols with >= 3 samples in the summary view (prune noise)
...
def playbook_body(tracker: dict, lessons: list[str], closed: list[dict],
    ...
    if lessons:
        ...
        lines.append("- (none)")
    lines.append("")
    lines.append("## Per-symbol record (dynamic-universe learning)")
    lines.append("")
    syms = (tracker.get("_symbols") or {})
    if syms:
        lines.append("| Symbol | n | Wins | Win rate | Edge $ | Verdict |")
        lines.append("|---|---|---|---|---|---|")
        for sym in sorted(syms, key=lambda s: -(syms[s].get("win_rate") or 0)):
            st = syms[sym]
            wr = st.get("win_rate")
            verdict = "⭐ KEEP (edge)" if (wr or 0) >= 0.6 else ("⚠️ DEMOTE" if (wr or 0) <= 0.35 else "hold")
            lines.append(f"| {sym} | {st['n']} | {st['wins']} | {util.fmt_pct(wr)} | "
                         f"{st['edge']:+.2f} | {verdict} |")
        lines.append("")
        lines.append("Symbols with ≥6 samples and win rate ≤35% are auto-flagged DEMOTE — "
                     "they get no per-symbol bonus and are candidates for removal from the core universe.")
    else:
        lines.append("- No per-symbol record yet (needs ≥3 graded trades per symbol).")
    lines.append("")
