...
def open_report(state: dict, research: dict, hyps: list[dict], opened: list[dict],
    ...
    dyn = (research.get("dynamic") or {})
    adds = dyn.get("adds") or []
    if adds:
        lines += ["## Dynamic universe additions", "",
                  f"- News/momentum rotation added: **{', '.join(adds)}** (scanned but "
                  "only traded if a ≥60% hypothesis forms).",
                  ""]
    sectors = dyn.get("sector_top") or []
    if sectors:
        sector_str = "; ".join("{} {:+.1f}%".format(s["etf"], s["ret5"] * 100) for s in sectors)
        lines += [f"- Sector rotation leaders: {sector_str}.",
                  ""]
