...
# ---------------------------------------------------------------------------
# Dynamic universe (Tier 2): news/SEC discovery + momentum rotation + sector RS
# ---------------------------------------------------------------------------
_STOP = set("""A AN AND ARE AS AT BE BY FOR FROM HAS HE HER HIS I IN IS IT ITS OF ON OR
THE THEY THIS TO WAS WE WHO WILL WITH YOU YOUR INC CO CORP ETF USA US""".split())


def extract_ticker_mentions(notes: list[dict], candidates: set[str]) -> list[str]:
    """Find candidate-pool tickers mentioned in RSS/news/inbox text."""
    import re
    found = set()
    pat = re.compile(r"\b[A-Z]{1,5}\b")
    for n in notes:
        text = " ".join(str(n.get(k, "")) for k in ("title", "summary"))
        for tok in pat.findall(text):
            if tok in _STOP or tok in ("AAPL", "MSFT", "NVDA", "TSLA", "AMD", "META",
                                       "AMZN", "GOOGL", "JPM", "XOM", "CVX", "SPY",
                                       "QQQ", "IWM", "GLD", "SLV", "USO", "TLT"):
                continue
...
def momentum_rotation(prices: dict, candidates: list[str], universe: list[str],
                      top_n: int = 6, min_volume: float = 500000) -> list[str]:
    """Top momentum/volume names from the candidate pool (5d momentum + volume)."""
    scores = {}
    for sym in candidates:
        if sym in universe:
            continue
        p = prices.get(sym)
        if not p or not p.get("bars") or len(p["bars"]) < 6:
            continue
        bars = p["bars"]
        ...
        if avg_vol < min_volume:
            continue
        # momentum dominant, volume as tiebreak
        scores[sym] = mom5 * 100 + min(0.02, avg_vol / 50_000_000)
    ranked = sorted(scores, key=lambda s: -scores[s])
    return ranked[:top_n]


def sector_rotation(prices: dict, sector_etfs: list[str], top_n: int = 3) -> list[dict]:
    """Rank sector ETFs by 5-day return → rotation leaders."""
    rs = []
    for etf in sector_etfs:
        p = prices.get(etf)
        if not p or not p.get("bars") or len(p["bars"]) < 6:
            continue
        closes = [b["c"] for b in p["bars"] if b.get("c")]
        if len(closes) < 6:
            continue
        ret5 = closes[-1] / closes[-6] - 1.0
        rs.append({"etf": etf, "ret5": round(ret5, 4)})
    rs.sort(key=lambda x: -x["ret5"])
    return rs[:top_n]


def compute_dynamic_universe(summary: dict, cfg: dict, universe: list[str]) -> dict:
    """Build the enriched scan set + metadata for this run."""
    notes = summary.get("notes") or []
    prices = summary.get("prices") or {}
    candidates = [s for s in cfg.get("candidate_pool", []) if s not in universe]
    cand_set = set(candidates)
    news_adds = extract_ticker_mentions(notes, cand_set) if cfg.get("auto_dynamic_universe", True) else []
    mom_adds = momentum_rotation(prices, candidates, universe,
                                 top_n=cfg.get("momentum_top_n", 6),
                                 min_volume=cfg.get("min_dynamic_volume", 500000)) \
        if cfg.get("auto_dynamic_universe", True) else []
    adds = []
    for s in news_adds + mom_adds:
        if s not in adds:
            adds.append(s)
    max_add = cfg.get("max_dynamic_additions", 12)
    adds = adds[:max_add]
    # sector rotation leaders (from universe sector ETFs)
    sector_etfs = ["XLE", "XLF", "XLK", "XLY", "XLV", "XLI", "XLB", "XLU", "XLP",
                   "XLC", "XLRE", "KRE", "IWM"]
    sector_etfs = [e for e in sector_etfs if e in universe or e in prices]
    top_sectors = sector_rotation(prices, sector_etfs, top_n=cfg.get("rotation_bias_top", 3))
    scan = list(universe) + adds
    return {"news_adds": news_adds, "momentum_adds": mom_adds,
            "sector_top": top_sectors, "adds": adds, "scan_universe": scan}
...
def run_research(cfg: dict, state_dir: Path, universe: list[str]) -> dict:
    ...
    #    fetch the base universe + the candidate pool so dynamic discovery has data
    candidates = cfg.get("candidate_pool", [])
    price_universe = list(dict.fromkeys(list(universe) + candidates + ["GC=F", "CL=F"]))
    ...
    # 7) dynamic universe (news discovery + momentum rotation + sector RS)
    dynamic = compute_dynamic_universe({"notes": notes, "prices": prices}, cfg, universe)
    scan = dynamic["scan_universe"]
    if dynamic["sector_top"]:
        top3 = ", ".join(f"{s['etf']} ({s['ret5']*100:+.1f}%)" for s in dynamic["sector_top"])
        notes.append({"category": "sector", "tickers": [s["etf"] for s in dynamic["sector_top"]],
                      "title": f"Sector rotation: {top3}",
                      "summary": f"Relative-strength leaders (5d): {top3}. "
                                 f"Rotation favors these sectors today.",
                      "direction": "bullish", "strength": 0.5, "source": "rotation",
                      "date": date.today().isoformat()})
        sources["sector_rotation"] = len(dynamic["sector_top"])
    util.log(f"dynamic universe: +{len(dynamic['adds'])} symbols "
             f"(news {len(dynamic['news_adds'])}, momentum {len(dynamic['momentum_adds'])}) "
             f"→ scanning {len(scan)}", "INFO")
