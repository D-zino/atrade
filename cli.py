...
def cmd_status(args) -> int:
    ...
    r = s.get("risk") or {}
    if r:
        print(f"Risk limits:  max positions {r.get('max_positions')} · per-position "
              f"{r.get('max_position_pct', 0)*100:.0f}% · portfolio cap "
              f"{r.get('max_portfolio_pct', 0)*100:.0f}% · stop {r.get('intraday_stop_pct', 0)*100:.1f}% · "
              f"daily loss {r.get('daily_loss_limit_pct', 0)*100:.0f}% · max drawdown "
              f"{r.get('max_drawdown_pct', 0)*100:.0f}% · peak equity "
              f"{r.get('peak_equity', 'n/a') if r.get('peak_equity') else 'n/a'}")
...
def cmd_config(args) -> int:
    """Show effective config, or set a key (persists to state/config.json).

    Usage:
      python3 -m atrade.cli config                  # show effective settings
      python3 -m atrade.cli config max_positions 5  # set one value
      python3 -m atrade.cli config reset            # clear all overrides
    """
    from . import engine
    st, cfg = engine._load_state_and_cfg(_state_dir_arg())
    overrides_path = st.dir / "config.json"
    if args.value == "reset" and args.key is None:
        if overrides_path.exists():
            overrides_path.unlink()
        print("config overrides cleared — back to defaults")
        return 0
    if args.key and args.value is not None:
        ...
        try:
            ...
            pass
        ov = util.read_json(overrides_path, {}) or {}
        ov[args.key] = val
        util.write_json(overrides_path, ov)
        print(f"set {args.key} = {val}  (persisted to {overrides_path})")
        return 0
    # show effective settings
    print("Effective configuration (defaults + state/config.json overrides):")
    keys = ["broker", "initial_equity", "min_confidence", "max_positions",
            "max_position_pct", "max_portfolio_pct", "min_trade_value", "max_trade_value",
            "conf_to_size", "intraday_stop_pct", "daily_loss_limit_pct",
            "max_drawdown_pct", "warmup_until_trades", "warmup_min_factor",
            "slippage_bps", "universe"]
    for k in keys:
        v = cfg.get(k)
    ...
    print()
    print("Examples:")
    print("  python3 -m atrade.cli config max_positions 5")
    print("  python3 -m atrade.cli config max_portfolio_pct 0.5")
    print("  python3 -m atrade.cli config intraday_stop_pct 0.02")
    print("  python3 -m atrade.cli config max_drawdown_pct 0.15")
    print("  python3 -m atrade.cli config reset")
    return 0
...
def main(argv=None) -> int:
    ...
    # positional extras belong to the command (universe symbols / config key=value)
    if args.command == "universe":
        args.symbols = extra
    else:
        args.symbols = []
    if args.command == "config":
        args.key = extra[0] if len(extra) >= 1 else None
        args.value = extra[1] if len(extra) >= 2 else None
    return {"open": cmd_open, "close": cmd_close, "status": cmd_status,
            ...
            "preview": cmd_preview, "weekahead": cmd_weekahead,
